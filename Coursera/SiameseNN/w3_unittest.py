...
def test_train_model(learner_func, model, lossfn):
    ...
    try: 
        trained_model = learner_func(model, lossfn, text_vectorization, train_gen, val_gen, epochs = 0)
    except Exception as e:
        t.failed = True
    ...
    try: 
        trained_model = learner_func(model, lossfn, text_vectorization, train_gen, val_gen, d_feature =16, lr = 0.1, epochs = 0)
    except Exception as e:
        t.failed = True
        t.msg = f"There was an error evaluating the `train_model` function."
        t.expected = "No exceptions"
